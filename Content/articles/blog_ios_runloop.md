...
3. `commonMode`
    ...
    一个 `Mode` 可以将自己标记为 `Common` 属性 (通过将其 `ModeName` 添加到 `RunLoop` 的 `commonModes` 中). 每当 RunLoop 的内容发生变化时, RunLoop 都会自动将添加到 `commomMode` 里的 `Source`/`Observer`/`Timer` 同步到具有 `Common` 标记的所有 `Mode` 里.

    `commonMode` 中默认已经添加了除 `UIInitializationRunloopMode` 以外的所有 mode(`UITrackingRunLoopMode`, `kCFRunLoopDefaultMode`, `GSEventReceiveRunLoopMode`, `kCFRunLoopCommonModes`), 可以创建自定义 `mode` 然后添加到 `commonMode`
...
- `source0`: 非基于 port, 主要是用户定义的事件, 比如点击, 触摸事件等. 需要手动唤醒线程, 将当前线程从内核态切换到用户态
- `source1`: 基于 port, 通过内核与其他线程接收与分发的事件. 能主动唤醒线程, 接受分发系统事件 (source1 在处理的时候会分发一些操作给 source0 去处理)
...
10. 通知观察者 `RunLoop` 结束.
    ...
    一个 `Mode` 可以将自己标记为" `Common` "属性 (通过将其 `ModeName` 添加到 `RunLoop` 的 `commonModes` 中). 每当 RunLoop 的内容发生变化时, RunLoop 都会自动将 添加到 `commomMode` 里的 Source/Observer/Timer 同步到具有 "Common" 标记的所有 Mode 里. 主线程的 mainRunLoop 预制了两个 mode, `defaultMode` 与 `UITrackingRunLoopMode`
